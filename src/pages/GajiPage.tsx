  ...
  return (
    <div className="space-y-6 px-1">
      ...
      <motion.div 
        ...
        className="flex flex-col p-5 sm:p-6 bg-white dark:bg-slate-950/60 border border-slate-900 rounded-3xl relative overflow-hidden space-y-6"
      >
        ...
        <div className="flex flex-col items-center text-center space-y-4">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-amber-500/10 to-emerald-500/10 border border-slate-200 dark:border-slate-800/80 flex items-center justify-center text-amber-400 shadow-xl shadow-slate-950/50 relative">
            <Sparkles className="w-6 h-6 text-amber-400 animate-bounce" style={{ animationDuration: '3s' }} />
            <ShieldAlert className="w-4 h-4 absolute -bottom-1 -right-1 text-emerald-400 bg-white dark:bg-slate-950 rounded-full" />
          </div>

          <div className="space-y-2 max-w-sm">
            <h3 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-wider">Sedang Disiapkan</h3>
        ...
        <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-900/30 border border-slate-900/80 space-y-4">
          <div className="flex items-center justify-between border-b border-slate-900/60 pb-2">
            <span className="text-[10px] font-black uppercase tracking-wider text-slate-600 dark:text-slate-400">Progres Pengembangan</span>
                ...
                <div className="flex flex-col items-center">
                  ...
                  {idx < steps.length - 1 && (
                    <div className={`w-[1px] h-10 my-1 ${
                      step.status === 'completed' ? 'bg-emerald-500/30' : 'bg-slate-100 dark:bg-slate-800'
                    }`} />
                  )}
                </div>
                <div className="space-y-0.5 min-w-0 flex-1">
                  <div className="flex items-center gap-1.5">
                    <p className={`text-xs font-bold leading-none ${
                      step.status === 'completed' ? 'text-slate-700 dark:text-slate-300' : step.status === 'current' ? 'text-sky-300 font-black' : 'text-slate-500 dark:text-slate-400'
                    }`}>
                      {step.name}
                    </p>
                    {step.status === 'current' && (
                  ...
                  <p className="text-[9.5px] text-slate-600 dark:text-slate-400 truncate leading-relaxed">
                    {step.desc}
                  </p>
                </div>
              </div>
        ...
        {/* Bottom tagline */}
        <div className="pt-2 text-center text-[10px] text-slate-500 dark:text-slate-400 font-bold tracking-wide uppercase flex items-center justify-center gap-1.5">
