...
export const AdminPage: React.FC = () => {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={refetch}
            disabled={isLoading}
            className="p-2.5 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:text-white cursor-pointer"
          >
      ...
      <GlassCard className="p-3.5 space-y-3 border-slate-200 dark:border-slate-800 shadow-md">
        <div className="flex flex-col gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-white dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 rounded-xl pl-10 pr-3 py-2.5 text-xs text-slate-900 dark:text-white outline-none focus:border-indigo-500 transition-colors"
            />
          </div>
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar pb-1">
            {['ALL', 'Pending', 'Active', 'Rejected', 'Suspended'].map((st) => (
              <button
                ...
                className={`px-3.5 py-2 rounded-xl text-[10px] sm:text-[11px] font-black uppercase tracking-wider whitespace-nowrap transition-all cursor-pointer ${
                  statusFilter === st
                    ? 'bg-indigo-500 text-slate-950 shadow-lg scale-[1.02]'
                    : 'bg-slate-50 dark:bg-slate-900/80 text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-800'
                }`}
              >
                {st}
              </button>
      ...
      <div className="space-y-3">
        {isLoading ? (
          ...
          <GlassCard className="py-12 text-center text-slate-600 dark:text-slate-400 space-y-2">
            ...
            <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">Tidak Ada Data Recruiter</p>
          ...
          paginatedUsers.map((user) => (
            <GlassCard
              key={user.telegramId}
              className={`p-4 space-y-3 border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:border-slate-700 ${
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  {user.photoUrl ? (
                    ...
                    <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-indigo-600 to-sky-500 flex items-center justify-center text-slate-900 dark:text-white font-bold text-sm">
              ...
              {/* Grid details */}
              <div className="bg-slate-50 dark:bg-slate-900/80 p-3 rounded-xl border border-slate-200 dark:border-slate-800/80 grid grid-cols-2 gap-2 text-xs">
                <div>
                  <span className="text-[10px] text-slate-500 dark:text-slate-400 font-semibold uppercase block">Email:</span>
                  <span className="text-slate-200 font-medium truncate block">{user.email}</span>
                </div>
                <div>
                  <span className="text-[10px] text-slate-500 dark:text-slate-400 font-semibold uppercase block">WhatsApp:</span>
                  <span className="text-slate-200 font-medium truncate block">{user.whatsapp}</span>
                </div>
                <div>
                  <span className="text-[10px] text-slate-500 dark:text-slate-400 font-semibold uppercase block">UID 9Kucing:</span>
                  <span className="text-slate-200 font-medium truncate block">{user.akun9Kucing}</span>
                </div>
                <div>
                  <span className="text-[10px] text-slate-500 dark:text-slate-400 font-semibold uppercase block">Didaftarkan:</span>
                  <span className="text-slate-700 dark:text-slate-300">{formatWIBDate(user.createdAt)}</span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-end gap-2 pt-1 border-t border-slate-200 dark:border-slate-800/60">
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-semibold flex items-center gap-1 cursor-pointer"
                >
                ...
                {user.status !== 'Suspended' && (
                  <button
                    ...
                    className="px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-amber-600/80 text-amber-300 text-xs font-semibold flex items-center gap-1 cursor-pointer"
                  >
      ...
      {/* Pagination Bar */}
      {totalPages > 1 && (
        <div className="flex items-center justify-between bg-slate-50 dark:bg-slate-900/80 p-3 rounded-2xl border border-slate-200 dark:border-slate-800 text-xs text-slate-600 dark:text-slate-400">
          <span>Halaman {currentPage} dari {totalPages}</span>
          <div className="flex items-center gap-2">
            <button
              disabled={currentPage === 1}
              onClick={() => setCurrentPage((p) => p - 1)}
              className="p-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 disabled:opacity-40 text-slate-900 dark:text-white"
            >
          ...
          <GlassCard className="max-w-md w-full space-y-4 text-left border-indigo-500/30">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
              ...
              <button
                ...
                className="text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:text-white font-bold"
              >
                ✕
              </button>
            </div>
            ...
            <div className="space-y-2 text-xs">
              <p><strong className="text-slate-600 dark:text-slate-400">Nama:</strong> {selectedUser.firstName} {selectedUser.lastName}</p>
              <p><strong className="text-slate-600 dark:text-slate-400">Username:</strong> {formatUsername(selectedUser.username)}</p>
              <p><strong className="text-slate-600 dark:text-slate-400">Telegram ID:</strong> {selectedUser.telegramId}</p>
              <p><strong className="text-slate-600 dark:text-slate-400">Email:</strong> {selectedUser.email}</p>
              <p><strong className="text-slate-600 dark:text-slate-400">WhatsApp:</strong> {selectedUser.whatsapp}</p>
              <p><strong className="text-slate-600 dark:text-slate-400">UID 9Kucing:</strong> {selectedUser.akun9Kucing}</p>
              <p><strong className="text-slate-600 dark:text-slate-400">Role:</strong> {selectedUser.role}</p>
            ...
            <div className="pt-3 border-t border-slate-200 dark:border-slate-800 space-y-2">
              <span className="text-[10px] text-slate-600 dark:text-slate-400 font-bold uppercase tracking-wider block">Ubah Status Recruiter:</span>
              <div className="flex flex-wrap gap-2">
                ...
                {selectedUser.status !== 'Suspended' && (
                  <button
                    ...
                    className="flex-1 min-w-[100px] px-3 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-amber-600/80 text-amber-300 text-[11px] font-bold flex items-center justify-center gap-1 cursor-pointer transition-colors"
                  >
                    <AlertTriangle className="w-3.5 h-3.5" /> Suspend
                  </button>
                )}
